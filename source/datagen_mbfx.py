...
GAIN_MIN = -10
GAIN_MAX = 10
OUT_PATH = pathlib.Path("/home/alexandre/dataset/mbfx_disto_guitar_mono_int")
...
for i in tqdm(range(NUM_RUNS), position=1):
    for file in tqdm(DATA_PATH.iterdir(), total=1872, position=0, leave=True):
        drive_levels = np.array([random.randint(DRIVE_MIN, DRIVE_MAX) for b in range(NUM_BANDS)])
        gain_levels = np.array([random.randint(GAIN_MIN, GAIN_MAX) for b in range(NUM_BANDS)])
